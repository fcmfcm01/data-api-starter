...
项目采用多模块结构（根 POM + starter + examples + spikes），从根目录构建即可：
...
这会将 starter 安装到本地 Maven 仓库，examples 下的示例项目可直接引用。

### 添加 Maven 依赖

在你的项目 `pom.xml` 中引入 starter。如果项目是同一多模块构建的子模块，可以使用 `${project.version}` 避免硬编码版本号：
...
此外需要 `spring-boot-starter-web`、`spring-boot-starter-jdbc`（或 `spring-boot-starter-data-r2dbc`）、`spring-boot-starter-actuator` 和 `springdoc-openapi-starter-webmvc-ui:3.0.3`，具体版本参考 `examples/order-service/pom.xml`。
...
如果使用 R2DBC 替代 JDBC，需要引入 `spring-boot-starter-data-r2dbc` 依赖和对应的 R2DBC 驱动（如 `r2dbc-h2`）。配置使用 `spring.r2dbc.*` 属性：
...
data-api:
  scope-mapping: "internal:basic+detail"
...
> **开发提示**：H2 支持 `MODE=MySQL`、`MODE=MSSQLServer` 等兼容模式，方便在本地开发和测试中模拟不同数据库的 SQL 方言。框架的 `SqlDialect.fromUrl()` 会根据 URL 参数自动选择正确的分页语法。
...
`page`（默认 1）和 `size`（默认 20）在 `parameters` 中声明即可。框架自动检测这两个参数名，根据 SqlDialect 构建对应数据库的分页 SQL：

- **MSSQL 方言**：`OFFSET N ROWS FETCH NEXT M ROWS ONLY`
- **MySQL / PostgreSQL / H2 方言**：`LIMIT M OFFSET N`

方言通过 `SqlDialect.fromUrl()` 从数据源 URL 自动检测，也支持 H2 的 `MODE=MSSQLServer` 兼容模式。
...
框架自动生成分页总数查询。`buildCountSql()` 根据方言移除对应的分片子句（`OFFSET ... FETCH` 或 `LIMIT ... OFFSET`），外包 COUNT：`SELECT COUNT(*) FROM (原始SQL去掉分片子句) AS _count`。与原始查询共享参数。
...
项目包含三个示例应用，分别展示不同的数据源类型和使用场景。

### 示例项目

| 示例 | 数据源类型 | 说明 |
|------|-----------|------|
| `examples/order-service/` | JDBC | 基础示例：订单 CRUD，分页查询，Scope 字段过滤 |
| `examples/r2dbc-product-service/` | R2DBC | R2DBC 示例：产品查询，使用 ConnectionFactory 和 MySQL 方言分页 |
| `examples/restclient-proxy-service/` | HTTP | HTTP 转发示例：代理 JSONPlaceholder API，展示 RestClient 请求转发 |

### 项目结构

```
data-api-starter/
├── starter/                  ← starter 模块（核心框架代码）
├── examples/                 ← 示例应用
│   ├── order-service/        ← JDBC 示例
│   ├── r2dbc-product-service/ ← R2DBC 示例
│   └── restclient-proxy-service/ ← HTTP 转发示例
└── spikes/                   ← spike 项目
```

### order-service 文件结构（JDBC 示例）
...
# 启动 R2DBC 示例
cd examples/r2dbc-product-service && mvn spring-boot:run

# 启动 HTTP 转发示例
cd examples/restclient-proxy-service && mvn spring-boot:run

# 验证（以 order-service 为例）
