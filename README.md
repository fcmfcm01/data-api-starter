...
```
YAML 文件 → YamlDiscovery → YamlParser → YamlLint → DynamicRouterRegistrar → Spring MVC 路由
```
...
**DynamicRouterRegistrar** 将校验通过的 `ApiDefinition` 动态注册为 Spring MVC 路由。请求到达时经过四层安全防护：`SqlInjectionGuard` 校验输入参数，`DdlGuard` 拦截未授权的 DDL 操作，`JdbcQueryEngine` 通过 `PreparedStatement` 参数化执行 SQL，最后 `ScopeFilter` 按权限裁剪响应字段。
